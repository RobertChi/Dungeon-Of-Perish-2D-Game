...
public class GoldenChest extends StaticEntity{
    ...
    public GoldenChest(Handler handler, float x, float y)
    {
        super(handler, x,y, 64, 64);

        chest=new BufferedImage[3];
        chest[0]= Assets.goldenchest[0];
        chest[1]= Assets.goldenchest[1];
        chest[2]= Assets.goldenchest[2];
        ...
        if(interacted==1 && System.currentTimeMillis()-time>800){
            ...
            if(intersected){
                if(handler.getKeyManager().interact){
                    ...
                    handler.getWorld().getEntityManager().getPlayer().score+=50;
                }
            }
            return;
        }
